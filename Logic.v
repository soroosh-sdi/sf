...
Inductive and (P Q : Prop) : Prop :=
  conj : P -> Q -> (and P Q).
Check conj.
Theorem evidence1 : forall x: nat , x=x.
Proof.
  intros. auto. Qed.

Check conj (forall x:nat, x=x)  (forall x:nat, x=x) evidence1 evidence1.
...
Proof.
  ...
  split. inversion IHn.  assumption.
  inversion IHn.
  intros.
  apply ev_SS. apply H. inversion H1. subst. unfold even. assumption.
Qed.
...
Proof.
  ...
  inversion Hl as [Hp | Hq].
    Case "P is true". left. apply Hp.
    Case "Q is true". 
        inversion Hr as [Hp' | Hr'].
           SCase "Again, P". left; apply Hp'.
           SCase "Again, Q". right. split; try apply Hq;try apply Hr'.
Qed.
...
Proof.
  ...
  unfold not.
  intros NotQ Pe .
  apply H in Pe. apply NotQ. assumption.
Show Proof.
 Qed.
  (* FILL IN HERE  Admitted. *)
(** [] *)
...
Proof. 
  ...
  induction n.
     intros. inversion H0.
     intros. apply IHn. inversion H0. assumption.
             assumption.
Qed.
  (* FILL IN HERE  Admitted. *)
(** [] *)
...
Print not.
Theorem boo: forall P : Prop,  (P \/ ~P) -> ~(~P /\ P) .
Proof.
intros. unfold not. unfold not in H.
inversion H; intros; inversion H1; apply H2 in H3; assumption.
Qed.

Theorem boo': forall P : Prop,  ~(~P /\ P) ->  (P \/ ~P) .
Proof.
 intros. unfold not. unfold not in H. right. intros. apply H.
 split. 
Admitted.
...
reflexivity.
Qed.
(* This proof inspired form 
https://github.com/flavioc/coq/blob/master/Logic.v
*)
 (* I can't*)
  (* FILL IN HERE  Admitted. *)
(** [] *)
